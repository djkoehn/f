    ...
    func drawNodes(cx: GraphicsContext, viewport: CGRect) {
        ...
        for (nodeIndex, node) in patch.nodes.enumerated() {
            ...
            if let image = node.image {
                // Resolve the image for drawing
                let resolvedImage = cx.resolve(image)
                
                // Draw the image with proper scaling to fit the node's rectangle
                cx.draw(resolvedImage, in: rect)
                
                // If selected, draw a highlight border
                if selected {
                    let bg = Path(roundedRect: rect, cornerRadius: layout.nodeCornerRadius)
                    cx.stroke(bg, with: .color(.white), style: .init(lineWidth: 2.0))
                }
                
                // For mouse hover feedback
                if rect.contains(toLocal(mousePosition)) {
                    let bg = Path(roundedRect: rect, cornerRadius: layout.nodeCornerRadius)
                ...
                // Original drawing code for rectangle-based nodes
                ...
                // Draw the title bar for the node
                var titleBar = Path()
            ...
            // Draw ports for both image and non-image nodes
