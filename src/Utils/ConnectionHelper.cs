...
public static class ConnectionHelper
{
    ...
    public static bool TryConnectBlock(Node context, IBlock block, Vector2 position)
    {
        ...
        var connected = gameManager.ConnectionManager.HandleBlockConnection(block, position);
        if (connected && block is BaseBlock baseBlock)
        {
            baseBlock.CompleteConnection();
            GD.Print(
                $"[ConnectionHelper Debug] Forced block {baseBlock.Name} to Connected state. Connection operation returned: {connected}");
        }
    ...
    public static void HighlightPipeAtPosition(Node context, Vector2 position)
    {
        ...
        else
        {
            ...
            // No need to clear highlights again as we already did it above
        }
    }
    ...
    {
        return block is BaseBlock baseBlock && baseBlock.HasOutputConnection();
    }
