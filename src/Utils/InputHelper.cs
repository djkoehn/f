...
public partial class InputHelper : Node
{
    ...
    public override void _Input(InputEvent @event)
    {
        ...
        if (@event is InputEventKey keyEvent && keyEvent.Pressed && keyEvent.Keycode == Key.Space)
        {
            ...
            var inputBlock = GetNode<BaseBlock>(SceneNodeConfig.GameManager.BlockLayer +
                                                "/BlockLayerViewport/BlockLayerContent/Input");
            if (inputBlock != null)
            {
    ...
    private void HandleRightClick(InputEventMouseButton mouseEvent)
    {
        ...
        if (block != null)
        {
            ...
            {
                GD.Print(
                    $"[Debug InputHelper] Block '{blockName}' is dragging; ending drag before returning to toolbar.");
    ...
    private void HandleLeftClick(InputEventMouseButton mouseEvent)
    {
        ...
        if (block.GetParent() is ToolbarBlockContainer container)
        {
            GD.Print(
                $"[Debug InputHelper] Block '{blockName}' is in the toolbar. Moving to BlockLayer before dragging.");
            ...
            if (block.GetParent() is Control ctrl) globalPos = ctrl.GlobalPosition + block.Position;
    ...
    private void HandleMouseMotion(InputEventMouseMotion mouseEvent)
    {
        if (_currentDraggedBlock != null)
        {
            ...
            {
                var effectivePos = _currentDraggedBlock.GetTokenPosition();
    ...
    private void PlaceBlock(BaseBlock block, Vector2 position)
    {
        ...
        var effectivePos = block.GetTokenPosition();
        GD.Print($"[Debug InputHelper] Attempting to connect block at position: {effectivePos}");
        var connected = ConnectionHelper.TryConnectBlock(this, block, effectivePos);
