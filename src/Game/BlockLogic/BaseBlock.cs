...
namespace F.Game.BlockLogic
{
    ...
    public partial class BaseBlock : Node2D, IBlock
    {
        ...
        public virtual void SetValue(float value) => _value = value;
        ...
        public override void _Ready()
        {
            ...
            GD.Print($"[Debug BaseBlock] {Name} metadata: {(Metadata != null ? $"ID={Metadata.Id}, SpawnOnSpace={Metadata.SpawnOnSpace}" : "null")}");
            AddToGroup("Blocks");
            InitializeDragging();
            InitializeSockets();
        ...
        private void InitializeManagers()
        {
            ...
            GD.Print($"  - TokenManager: {(_tokenManager != null ? "found" : "null")}");
            
            if (_tokenManager == null)
            {
                GD.PrintErr($"[Debug BaseBlock] {Name} failed to get TokenManager from GameManager, will retry");
                // Retry after a short delay
                CreateTimer(0.1f, nameof(InitializeManagers));
                return;
            }
        ...
        public void SpawnToken()
        {
            ...
            GD.Print($"[BaseBlock Debug] TokenManager exists: {_tokenManager != null}");
            ...
            GD.Print($"[BaseBlock Debug] Metadata: {(Metadata != null ? $"Id={Metadata.Id}, SpawnOnSpace={Metadata.SpawnOnSpace}" : "null")}");
            GD.Print($"[BaseBlock Debug] Block state: {State}");
            
            if (_tokenManager == null)
            {
                GD.PrintErr($"[BaseBlock Debug] Failed to spawn token - TokenManager is null");
                return;
            }
        ...
        public virtual void ProcessToken(Token token)
        {
            ...
            if (!string.IsNullOrEmpty(Metadata?.ProcessTokenScript))
            {
                try
                {
                    float Value = token.Value; // Create local Value variable for the script
            ...
            {
                GD.Print($"[BaseBlock Debug] No next block found, destroying token");
                token.QueueFree();
            }
        }
