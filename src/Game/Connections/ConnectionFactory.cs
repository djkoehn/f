...
public sealed class ConnectionFactory
{
    ...
    public ConnectionPipe CreateConnection(IBlock from, IBlock to)
    {
        ...
        {
            GD.PrintErr(
                $"Could not find required sockets! Output: {outputSocket != null}, Input: {inputSocket != null}");
            return null;
        }
    ...
    public static ConnectionPipe CreatePipeForInsertion(IBlock from, IBlock to)
    {
        ...
        GD.Print(
            $"[ConnectionFactory] From socket type: {outputSocket.GetType()}, To socket type: {inputSocket.GetType()}");
